...
def calc_W_gradient(model, t, dQ, alpha, eps):
    ...
    e = reduce(lambda x, (k, v): x + v.a * dQ[k][t] ** 2 + v.b * dQ[k][t] + v.c, model_fix_t.edges.items(), 0)
    tw = (pr + cs + prt) - alpha * e
...
def calc_gradient(model, T, dQ, i, alpha, eps):
    w_gradient_by_t = list((calc_W_gradient(model, t, dQ, alpha, eps) for t in range(0, T)))
    return {
        'gradient': {
            k: list(
                (reduce(lambda x, tau: x + calc_dt(i, tau, T) * w_gradient_by_t[tau]['gradient'][k], range(t, T - 1), 0)
                 - alpha * calc_dt(i, t, T) * (2 * v.a * dQ[k][t] + v.b) if (model.indicators[k][t] > 0) else 0 for t in range(0, T)))
...
def calc_conditions(model, T, dQ, i, alpha, eps):
    w_gradient_by_t = list((calc_W_gradient(model, t, dQ, alpha, eps) for t in range(0, T)))
    return {
        k: list(
                (reduce(lambda x, tau: x + calc_dt(i, tau, T) * w_gradient_by_t[tau]['gradient'][k], range(t, T - 1), 0)
...
def start_external_procedure(model, T, eps, c, i = 0.1, alpha=0.02, q_initial = None, projector=lambda x: x):
    ...
    while True:
        dq_prev = dq
        gradient = calc_gradient(model, T, dq, i, alpha, eps)
